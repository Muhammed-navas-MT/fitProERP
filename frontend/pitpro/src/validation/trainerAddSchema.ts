...
export const trainerSignupSchema = z
  .object({
    ...
    sessionCount: z
      .number({ error: "Session count must be a number" })
      .int("Session count must be an integer")
      .min(0, "Session count cannot be less than 0"),

    allocatedLeaveCount: z
      .number({ error: "Allocated leave count must be a number" })
      .int("Allocated leave count must be an integer")
      .min(0, "Allocated leave count cannot be less than 0"),
  ...
  .superRefine((data, ctx) => {
    ...
    const start = toMinutes(data.dutyTime.startTime);
    const end = toMinutes(data.dutyTime.endTime);
    const workingMinutes = end - start;
    const maxSessions = Math.floor(workingMinutes / 60);

    if (data.sessionCount > maxSessions) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["sessionCount"],
        message: "Session count cannot exceed working hours",
      });
    }
  });

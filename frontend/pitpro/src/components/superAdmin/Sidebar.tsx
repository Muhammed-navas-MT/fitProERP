...
export default function Sidebar({
  ...
  const menuItems = [
    ...
    },
    {
      icon: Wallet,
      label: "Payments",
      href: `${FRONTEND_ROUTES.SUPER_ADMIN.BASE}/${FRONTEND_ROUTES.SUPER_ADMIN.LIST_PAYMENTS}`,
    },
  ];
        ...
        >
          ...
          >
            <ul className="space-y-2">
              {menuItems.map((item) => (
                <li key={item.label}>
                  <Link to={item.href} className={getLinkClasses(item.href)}>
                    <item.icon size={20} />
    ...
    >
      ...
      <nav className="flex-1 p-4">
        <ul className="space-y-2">
          {menuItems.map((item) => (
            <li key={item.label}>
              <a href={item.href} className={getLinkClasses(item.href)}>
                <item.icon size={20} />
                <span>{item.label}</span>
              </a>
            </li>

...
import {
  useListDiet,
  useCreateDietPlan,
} from "@/hook/member/dietPlanHooks";
...
export default function DietPlanPage() {
  ...
  const { data, isLoading, isError } = useListDiet();
  const { mutate: createDiet } = useCreateDietPlan();

  const diet: listDietPlanResponseDto | undefined =
    data?.data;
  ...
  const selectedDietDay: DietDay =
    diet.days.find((d) => d.day === selectedDay) ||
    diet.days[0];

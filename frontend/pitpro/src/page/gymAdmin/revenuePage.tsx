...
export default function RevenuePage() {
  ...
  const [selectedRevenue, setSelectedRevenue] = useState<IListRevenueItemType|null>(null);
  ...
  const {
    ...
    summary.forEach((item:{count:number,sourceType:string,totalAmount:number}) => {
  ...
  const handleView = (id: string) => {
    const found = revenueData.find((rev:IListRevenueItemType) => rev.id === id);
    if (found) {
  ...
  if (isLoading) return <RevenueTableSkeleton />;
  if (isError) return <RevenueErrorState onRetry={refetch} />;

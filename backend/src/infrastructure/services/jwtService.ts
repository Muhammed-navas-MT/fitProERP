import { sign, SignOptions, verify, decode as jwtDecode } from "jsonwebtoken";
...
export class JwtService implements IJwtService {
    ...
    verifyAccessToken(token: string): JWTPayloadType | null {
        try {
            ...
            const { id, role, subdomain } = verify(token, secretKey) as JWTPayloadType;
            if (!(id && role)) {
                throw new TokenExpiredException(Error.TOKEN_INVALID);
            }
            return { id, role, subdomain };
        } catch (error) {
            throw new TokenExpiredException(Error.TOKEN_EXPIRED);
        }
    }

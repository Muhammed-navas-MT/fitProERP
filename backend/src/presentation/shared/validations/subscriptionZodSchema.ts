...
export const subscriptionSchema = z.object({
  ...
  price: z
    .number({ error: SubscriptionError.PRICE_INVALID_TYPE })
    .nonnegative(SubscriptionError.PRICE_NEGATIVE)
    .min(9999, SubscriptionError.PRICE_TOO_SHORT),

  duration: z.enum(
    [Durations.ONE_MONTH, Durations.ONE_YEAR, Durations.TREE_MONTHS],
    {
  ...
  features: z
    ...
    .refine(
      (features) =>
        new Set(features.map((f) => f.trim().toLowerCase())).size ===
        features.length,
      { error: SubscriptionError.FEATURE_DUPLICATE },
    ),

  isActive: z.boolean({ error: SubscriptionError.IS_ACTIVE_INVALID_TYPE }),
});

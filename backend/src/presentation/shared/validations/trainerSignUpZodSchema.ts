...
export const trainerSignupSchema = z
  .object({
    ...
    sessionCount: z
      .number({ error: TrainerError.SESSION_COUNT_INVALID_TYPE })
      .int({ error: TrainerError.SESSION_COUNT_INVALID_TYPE })
      .min(0, { error: TrainerError.SESSION_COUNT_MIN }),

    allocatedLeaveCount: z
      .number({ error: TrainerError.ALLOCATED_LEAVE_INVALID_TYPE })
      .int({ error: TrainerError.ALLOCATED_LEAVE_INVALID_TYPE })
      .min(0, { error: TrainerError.ALLOCATED_LEAVE_MIN }),
  ...
  .superRefine((data, ctx) => {
    const { startTime, endTime } = data.dutyTime;
    ...
    if (data.sessionCount > maxSessions) {
      ctx.addIssue({
        ...
        message: TrainerError.SESSION_COUNT_EXCEEDS_WORKING_HOURS,
      });
    }
  });

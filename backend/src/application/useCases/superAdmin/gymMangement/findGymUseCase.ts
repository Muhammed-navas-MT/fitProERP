...
export class FindGymUseCase implements IFindGymUseCase {
  ...
  async findGym(id: string): Promise<IGymAdminDetailDTO | null> {
    ...
    const subscription = gymAdmin.packageId
      ? await this._subscriptionRepository.findById(gymAdmin.packageId)
      : null;
    if (!gymAdmin._id) {
      throw new ForbiddenException("Gym ID is missing...");
    }
    const trainersCount = await this._trainerRepository.countTrainersByGymId(
      gymAdmin._id,
    );
    const membersCount = await this._memberRepository.countMembersByGymId(
      gymAdmin._id,
    );

    const response = GymAdminMapper.mapGymAdminToDetailDTO(
      gymAdmin,
      subscription,
      membersCount,
      trainersCount,

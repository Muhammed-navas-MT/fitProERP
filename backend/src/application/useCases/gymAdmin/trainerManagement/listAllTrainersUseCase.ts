...
} from "../../../dtos/gymAdminDto/listAllTrainersDto";
...
export class listAllTrainersUseCase implements IListAllTrainersUseCase {
  ...
  async listAllTrainers(
  ...
  ): Promise<IListTrainerResponseDTO | null> {
    ...
    let { trainers, total } =
      await this._trainerRepository.listAllTrainers(params);

    const response: IListTrainerResponseDTO =
      TrainerMapper.toListTrainersResponse(trainers, total, params);
    return response;
  }
}

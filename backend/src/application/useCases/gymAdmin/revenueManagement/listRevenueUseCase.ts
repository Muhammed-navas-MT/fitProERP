...
export class ListRevenueUseCase implements IListRevenueUseCase {
  ...
  ): Promise<IListRevenueResponseDTO> {
    const { revenues, total, grandTotalAmount, summary } =
      await this._revenueRepository.findAllRevenues(params);
    const response = RevenueMapper.toRevenueListResponse(
      revenues,
      params,
      total,
      summary,
